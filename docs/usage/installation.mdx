...
your cluster. The simplest way to install OLM on your cluster is:
...
> Notes:
>
> * For now on, only the `alpha` channel is available. It is frequently
>   updated but might contain imcompatibles changes between releases.
> * This subscription assumes the operator is upgraded automatically.
>   to perform manual upgrade, use the `installPlanApproval` and 
>   `startingCSV` properties.
