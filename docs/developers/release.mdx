...
Once the agent built, you must change the reference in the
`operator/pkg/controller/instance/instance_controller.go` file. Search for
the `tag := "` in the file. 

## Build and release Controllers

see `make build` in the operator directory

## Build and release the Operator

see `make bundle` in the operator directory

## Build and release the Registry

see `make build` in the `registry` directory

## Check the Operator get updated

You can check your subscription to check that the Operator has been updated.
